//"DeFi Vault has already surpassed 50 million active users globally in just six months since launch. We’re on track to reach 100 million by the end of this year, making us the largest decentralized finance platform by active users."
﻿<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>À propos</title>
  <link rel="stylesheet" href="bootstrap.css">
  <link rel="stylesheet" href="style.css">
  <link rel="icon" href="img/logo.png">
</head>

<body>
  <!-- nav bar begins now !-->
  <nav class="navbar sticky-top navbar-expand-lg navbar-dark bg-nav">
    <div class="container-fluid">
      <a class="navbar-brand logoPosition" href="home.html"><img src="img/logo.png" class="img-logo"></a>
      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarText"
        aria-controls="navbarText" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
      </button>

      <div class="collapse navbar-collapse alingnementText " id="navbarText">


        <ul class="navbar-nav me-auto mb-2 mb-lg-0 ">
          <li class="nav-item textPosition ">
            <a class="nav-link active " aria-current="page" href="home.html">Home</a>
          </li>
         
          <li class="nav-item textPosition">
            <a class="nav-link " aria-current="page" href="about.html">À propos</a>
          </li>
          <li class="nav-item textPosition ">
            <a class="nav-link  " aria-current="page" href="Contact.html">Contact</a>
          </li>
          
          <li class="nav-item textPosition">
            <a class="nav-link btn " aria-current="page" type="submit" href="login.php">sign-in</a>
          </li>
        </ul>

        
      </div>
    </div>
  </nav>
  <!--********************about-section******************-->
  <section class="our-speaker-area bg-img bg-gradient-overlay section-padding-100-60" style="background-color: #820000;"
    id="team">
    <div class="about-section-center">
      <h1 class="textSize2" style="color: orange;">TYM JUNIOR ENTERPRISE</h1>
      <p class="textSize3">Tunisian Youth Marketers est la junior entreprise de
        "Ecole supérieure de commerce Tunis"</p> <p class="textSize3"> ESCT Manouba
        C'est un cabinet de conseil spécialisé en marketing
        qui offre</p> <p class="textSize3">une variété de services
        qui répondent aux attentes du client
        comme </p> <p class="textSize3">les études de notoriété,
        Faisabilité, satisfaction client,
        Et gestion communautaire.
      </p>
      
      
    </div>
    <!--*****************team-section**********************-->

    <div data-aos="fade-up" data-aos-duration="1000">
      <div class="container" style="padding-top: 50px;">
        <div class="row">
          <!-- Heading -->
          <div class="col-12">
            <div class="section-heading text-center wow fadeInUp" data-wow-delay="300ms"
              style="visibility: visible; animation-delay: 300ms; animation-name: fadeInUp;">

              <h4 style="color: orange;;font-weight:700;padding-bottom: 50px;">NOTRE EQUIPE</h4>
            </div>
          </div>
        </div>

        <div class="row">
          <!-- Single Speaker Area -->
          <div class="col-12 col-md-6 col-lg-4">
            <div class="single-speaker-area bg-gradient-overlay-2 wow fadeInUp" data-wow-delay="300ms"
              style="visibility: visible; animation-delay: 300ms; animation-name: fadeInUp;">
              <!-- Thumb -->
              <div class="speaker-single-thumb">
                <img src="img/team.jpg" alt="" class="img8">
              </div>


              <!-- Info -->
              <div class="speaker-info">
                <h5>eafea afeaf</h5>
                <p>President</p>
              </div>
            </div>
          </div>

          <!-- Single Speaker Area -->
          <div class="col-12 col-md-6 col-lg-4">
            <div class="single-speaker-area bg-gradient-overlay-2 wow fadeInUp" data-wow-delay="300ms"
              style="visibility: visible; animation-delay: 300ms; animation-name: fadeInUp;">
              <!-- Thumb -->
              <div class="speaker-single-thumb">
                <img src="img/team.jpg" alt="" class="img8">
              </div>

              <div class="speaker-info">
                <h5>eaeaf eafefea</h5>
                <p>Vice President</p>
              </div>
            </div>
          </div>

          <!-- Single Speaker Area -->
          <div class="col-12 col-md-6 col-lg-4">
            <div class="single-speaker-area bg-gradient-overlay-2 wow fadeInUp" data-wow-delay="300ms"
              style="visibility: visible; animation-delay: 300ms; animation-name: fadeInUp;">
              <!-- Thumb -->
              <div class="speaker-single-thumb">
                <img src="img/team.jpg" alt="" class="img8">
              </div>


              <!-- Info -->
              <div class="speaker-info">
                <h5>afeafefeaf afeafea</h5>
                <p>General Secretary</p>
              </div>
            </div>
          </div>

          <!-- Single Speaker Area -->
          <div class="col-12 col-md-6 col-lg-4">
            <div class="single-speaker-area bg-gradient-overlay-2 wow fadeInUp" data-wow-delay="300ms"
              style="visibility: visible; animation-delay: 300ms; animation-name: fadeInUp;">
              <!-- Thumb -->
              <div class="speaker-single-thumb">
                <img src="img/team.jpg" alt="" class="img8">
              </div>

              <!-- Info -->
              <div class="speaker-info">
                <h5>aefafa eafafeaf </h5>
                <p>Project Manager</p>
              </div>
            </div>
          </div>

          <!-- Single Speaker Area -->
          <div class="col-12 col-md-6 col-lg-4">
            <div class="single-speaker-area bg-gradient-overlay-2 wow fadeInUp" data-wow-delay="300ms"
              style="visibility: visible; animation-delay: 300ms; animation-name: fadeInUp;">
              <!-- Thumb -->
              <div class="speaker-single-thumb">
                <img src="img/team.jpg" alt="" class="img8">
              </div>

              <!-- Info -->
              <div class="speaker-info">
                <h5>eafeeaf aefeafeaf</h5>
                <p>Human Resources Manager</p>
              </div>
            </div>
          </div>

          <!-- Single Speaker Area -->
          <div class="col-12 col-md-6 col-lg-4">
            <div class="single-speaker-area bg-gradient-overlay-2 wow fadeInUp" data-wow-delay="300ms"
              style="visibility: visible; animation-delay: 300ms; animation-name: fadeInUp;">
              <!-- Thumb -->
              <div class="speaker-single-thumb">
                <img src="img/team.jpg" alt="" class="img8">
              </div>
              <!-- Social Info -->
              <div class="social-info">
                <a href="https://www.facebook.com/"><i class="zmdi zmdi-facebook"></i></a>
                <a href="https://instagram.com/"><i class="zmdi zmdi-instagram"></i></a>
                <a href="https://www.linkedin.com/in/"><i class="zmdi zmdi-linkedin"></i></a>
              </div>
              <!-- Info -->
              <div class="speaker-info">
                <h5>momomo momo</h5>
                <p>Business Development Manager</p>
              </div>
            </div>
          </div>


          <!-- Single Speaker Area -->


        </div>

      </div>
  </section>
  </div>

  <!--*********************foooooooooooter-section*******-->
  <footer id="footer" class="footer-1">
    <div class="main-footer widgets-dark typo-light">
      <div class="container">
        <div class="row">

          <div class="col-xs-12 col-sm-6 col-md-3">
            <div class="widget subscribe no-box">
              <h5 class="widget-title">Tunisian Youth Marketers - TYM<span></span></h5>
              <p>TYM est la Junior Entreprise au sein de l Ecole Supérieure De Commerce .Il s’agit d’un cabinet de
                conseil spécialisé en Marketing .</p>
            </div>
          </div>

          <div class="col-xs-12 col-sm-6 col-md-3">
            <div class="widget no-box">
              <h5 class="widget-title">nos service<span></span></h5>
              <ul class="thumbnail-widget">
                <li>
                  <div class="thumb-content"><a href="#.">gestionnaire de communauté</a></div>
                </li>
                <li>
                  <div class="thumb-content"><a href="#.">étude de notoriété
                    </a></div>
                </li>
                <li>
                  <div class="thumb-content"><a href="#.">étude de satisfaction
                    </a></div>
                </li>
                <li>
                  <div class="thumb-content"><a href="#.">étude de faisabilité
                    </a></div>
                </li>


              </ul>
            </div>
          </div>

          <div class="col-xs-12 col-sm-6 col-md-3">
            <div class="widget no-box">
              <h5 class="widget-title">Get Started<span></span></h5>
              <p>Get access to your full Training and Marketing Suite.</p>
              <a class="btn" type="submit" action="" href="#." target="_blank">Register Now</a>
            </div>
          </div>

          <div class="col-xs-12 col-sm-6 col-md-3">

            <div class="widget no-box">
              <h5 class="widget-title">Contact Us<span></span></h5>

              <p><a href="mailto:tymje@tymjuniorentreprise.com" style="color:#ff8d1e"
                  title="glorythemes">tymje@tymjuniorentreprise.com</a></p>
              <ul class="social-footer2">
                <li class=""><a title="youtube" target="_blank"
                    href="https://www.linkedin.com/company/tunisian-youth-marketers-tym/"><img alt="youtube" width="30"
                      height="30" src="img/33.png"></a></li>
                <li class=""><a href="https://www.facebook.com/TYM.JE.ESC.Mannouba " target="_blank"
                    title="Facebook"><img alt="Facebook" width="30" height="30"
                      src="data:image/png;base64,"></a>
                </li>
                <li class=""><a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30"
                      height="30"
                      src="data:image/png;base64,"></a>
                </li>
                <li class=""><a title="instagram" target="_blank"
                    href="https://www.instagram.com/tunisian_youth_marketers/?hl=fr "><img alt="instagram" width="30"
                      height="30"
                      src="data:image/png;base64,"></a>
                </li>
              </ul>
            </div>
          </div>

        </div>
      </div>
    </div>

    <div class="footer-copyright">
      <div class="container">
        <div class="row">
          <div class="col-md-12 text-center">

          </div>
        </div>
      </div>
    </div>
  </footer>
  <script src="bootstrapjava.js"></script>
  <script src="java.js"></script>
</body>

</html>
